...
-- Learning note: I need to save files to the postgre data folder, they can't be copied from the desktop or downloads folder.
...
alter column dim_id type varchar(30)

alter table score_meta
alter column dimension type varchar(36)

alter table score_meta
alter column subdim type varchar(30)
--- Learning notes: There's got to be a better way to determine max length for variables, maybe I could write a script that would take in a column and return the row with the max length. 
copy score_meta (
	scoreId,
	dimension,
	dim_id,
...
-----Admittedly, I probably did this in a roundabout way because I was indecisive about including the age and tenure variables because of all the missing values. I added them back in, but the file was too large to add to tableau public, 
-----so I ended up using both the "new_data" and "emps" tables. 
